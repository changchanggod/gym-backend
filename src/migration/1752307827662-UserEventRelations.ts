...
export class UserEventRelations1752307827662 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'CREATE TABLE `users_join_events_events` (`usersId` int NOT NULL, `eventsId` int NOT NULL, INDEX `IDX_6b35ef4a0e02617ab4a7ce9110` (`usersId`), INDEX `IDX_dc81dfc155e291a8004631fa22` (`eventsId`), PRIMARY KEY (`usersId`, `eventsId`)) ENGINE=InnoDB'
    );
    ...
    await queryRunner.query(
      'ALTER TABLE `events` ADD `organizerId` int NOT NULL'
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      'ALTER TABLE `users_join_events_events` ADD CONSTRAINT `FK_6b35ef4a0e02617ab4a7ce9110d` FOREIGN KEY (`usersId`) REFERENCES `users`(`id`) ON DELETE CASCADE ON UPDATE CASCADE'
    );
    await queryRunner.query(
      'ALTER TABLE `users_join_events_events` ADD CONSTRAINT `FK_dc81dfc155e291a8004631fa22c` FOREIGN KEY (`eventsId`) REFERENCES `events`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION'
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'ALTER TABLE `users` ADD `hostEventId` text NOT NULL'
    );
    await queryRunner.query(
      'ALTER TABLE `users` ADD `joinEventId` text NOT NULL'
    );
    await queryRunner.query(
      'ALTER TABLE `events` ADD `organizer` int NOT NULL'
    );
    await queryRunner.query(
      'ALTER TABLE `events` ADD `participants` text NOT NULL'
    );
    await queryRunner.query(
